...
def main() -> int:
    ...
    # Modified to use reduced feature matrix for testing feature selection improvement
    reduced_matrix_path = integrator.features_dir / "reduced_feature_matrix_top50.csv"
    if reduced_matrix_path.is_file():
        print("Using reduced feature matrix for modeling")
        feature_matrix = pd.read_csv(reduced_matrix_path)
        # Load combined_df for compatibility with the rest of the pipeline
        combined_df = pd.read_csv(integrator.features_dir / "combined_feature_matrix.csv")
    elif args.skip_feature_refresh and (integrator.features_dir / "modeling_feature_matrix.csv").is_file():
        combined_df = pd.read_csv(integrator.features_dir / "combined_feature_matrix.csv")
